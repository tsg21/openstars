...
const testGalaxy: Galaxy = {
  ...
  planets: [
    ...
    { id: "PL000002", name: "B", x: 100 * LIGHT_YEAR, y: 0 },
    { id: "PL000003", name: "C", x: 0, y: 100 * LIGHT_YEAR },
    { id: "PL000004", name: "D", x: 100 * LIGHT_YEAR, y: 100 * LIGHT_YEAR },
  ],
};
...
describe("computeInitialCentre", () => {
  it("centres on a specific planet when ID is provided", () => {
    ...
    expect(centre.centreX).toBe(100 * LIGHT_YEAR);
  ...
  it("falls back to galaxy centroid when no planet ID given", () => {
    ...
    expect(centre.centreX).toBe(50 * LIGHT_YEAR);
    expect(centre.centreY).toBe(50 * LIGHT_YEAR);
  });
...
describe("FIXED_SCALE", () => {
  ...
  it("shows ~400 light-years across 1000px", () => {
    const lightYearsIn1000px = 1000 / (FIXED_SCALE * LIGHT_YEAR);
    expect(lightYearsIn1000px).toBeCloseTo(400, 0);
  });
});
