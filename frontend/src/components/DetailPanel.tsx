...
export function DetailPanel({ collapsed, onToggle }: DetailPanelProps) {
  return (
    <aside
      className={`relative border-l border-[var(--color-panel-border)] bg-[var(--color-panel-bg)] transition-[width] duration-200 ${
        collapsed ? "w-0 overflow-hidden border-l-0" : "w-[350px]"
      }`}
    >
      {/* Toggle button */}
      <button
