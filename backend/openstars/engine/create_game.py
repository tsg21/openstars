...
def create_initial_state(
    ...
    player_colony_design_id: dict[str, str] = {}
    for player in players:
        ...
        colony_design_id, next_id = allocate_id(next_id, game_seed, "DE")
        designs.append(
            Design(
                id=colony_design_id,
                owner=player.username,
        ...
        player_colony_design_id[player.username] = colony_design_id

    # Create one scout fleet, one small freighter fleet, and one colony ship
    # fleet per player at home planet.
    fleets = []
    for i, player in enumerate(players):
        ...
        colony_fleet_id, next_id = allocate_id(next_id, game_seed, "FL")
        fleets.append(
            Fleet(
                id=colony_fleet_id,
                owner=player.username,
                position=Position(x=home_planet.x, y=home_planet.y),
                composition=[
                    FleetComposition(design_id=player_colony_design_id[player.username], count=1)
                ],
                cargo=Cargo(),
                waypoints=[],
                repeat=False,
