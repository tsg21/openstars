...
def resolve_current_turn(
    ...
    turn_token = turn.set(current_turn)
    try:
        with log_duration(log, "resolution.load_state"):
        ...
        with log_duration(log, "resolution.engine"):
        ...
        try:
            with log_duration(log, "resolution.persist_global_state"):
                storage.create_global_state(game_id, new_turn, new_state)
        except FileExistsError:
            log.warning("resolution.turn_already_exists new_turn=%d", new_turn)
        ...
        with log_duration(log, "resolution.derive_player_states"):
            for p in players:
                previous_player_state = (
                    None
                    if current_turn == 0
                    else storage.load_player_state(game_id, p, current_turn)
                )
        ...
        with log_duration(log, "resolution.turn_resolved_notify"):
            directory.turn_resolved(game_id, new_turn)
