...
class MemoryStorage(GameStorage):
    ...
    def save_galaxy(self, game_id: str, galaxy: Galaxy) -> None:
        key = game_object_name(game_id, "galaxy.json.gz")
    ...
    def save_global_state(self, game_id: str, turn: int, state: GlobalState) -> None:
        key = game_object_name(game_id, "state", f"global-state-T{turn}.json.gz")
    ...
    def save_player_state(self, game_id: str, username: str, turn: int, state: PlayerState) -> None:
        ...
        key = game_object_name(game_id, "players", f"player-state-{username}-T{turn}.json.gz")
        self._put_json(key, state.model_dump_json(indent=2))
    ...
    def save_commands(
        ...
        validate_segment(username, "username")
        key = game_object_name(game_id, "commands", f"player-command-{username}-T{turn}.json.gz")
    ...
    def list_games(self) -> list[str]:
        game_ids = {
            key.split("/", maxsplit=1)[0] for key in self._objects if key.endswith("/meta.json.gz")
        }
        return sorted(game_ids)

    def save_game_meta(self, game_id: str, meta: dict) -> None:
        key = game_object_name(game_id, "meta.json.gz")
    ...
    def save_design(self, game_id: str, username: str, design: Design) -> None:
        validate_segment(username, "username")
        key = game_object_name(game_id, "designs", username, f"{design.id}.json.gz")
        self._put_json(key, design.model_dump_json(indent=2))

    def load_design(self, game_id: str, username: str, design_id: str) -> Design:
        validate_segment(username, "username")
        key = game_object_name(game_id, "designs", username, f"{design_id}.json.gz")
    ...
    def list_designs(self, game_id: str, username: str) -> list[Design]:
        ...
        design_keys = sorted(
            key for key in self._objects if key.startswith(prefix) and key.endswith(".json.gz")
        )
    ...
    def save_combat_log(self, game_id: str, battle_id: str, log: CombatLog) -> None:
        ...
        key = game_object_name(game_id, "combat", f"{battle_id}.json.gz")
    ...
    def list_combat_logs(self, game_id: str) -> list[str]:
        ...
        return sorted(
            key[len(prefix) :].removesuffix(".json.gz")
