...
- [ ] Change keys from `…/foo.json` to `…/foo.json.gz` so that `MemoryStorage` mirrors the production layout (anything that does prefix matching across adapters stays consistent — list_games, etc.)
- [ ] Update `list_games` in memory.py to match the `.json.gz` suffix
...
- [ ] Wipe any local dev storage directories (typically `backend/data/` or wherever `STORAGE_BACKEND=local` writes) before running the new code
