...
- [x] Add `last_scanned_turn: int | None = None` to `PlayerPlanet` in `backend/openstars/engine/models.py`
- [x] Update the `scan_level` comment on `PlayerPlanet` to include `"stale"`
- [x] Update `derive_player_state` signature in `backend/openstars/engine/fog.py` to accept `previous_player_state: PlayerState | None = None`
- [x] In `derive_player_state`, build a lookup `prev_planets: dict[str, PlayerPlanet]` from `previous_player_state.planets` (keyed by planet ID) when the previous state is provided
- [x] In the `else` (scan_level `"none"`) branch of planet derivation, check `prev_planets`:
...
- [x] Unit tests in the existing fog-of-war test file (or alongside it):
...
- [x] In `backend/openstars/server/routes/play.py`, in the resolve endpoint, load the current player state (`turn N`) before calling `derive_player_state` for turn `N+1` — pass it as `previous_player_state`
- [x] Handle the `FileNotFoundError` case gracefully: if no previous player state exists (turn 0 edge case), pass `None`
- [x] Game creation (`backend/openstars/server/routes/games.py`) passes `None` as `previous_player_state` — no change needed, but verify the call still works
...
- [x] Write `test_stale_planet_after_fleet_moves_away` in `TestScanners`:
...
- [x] Add `"stale"` to the `ScanLevel` union in `frontend/src/types/game.ts`
- [x] Add `lastScannedTurn?: number` to the `PlayerPlanet` type
- [x] In `frontend/src/components/PlanetDetail.tsx`, add a `scan_level: "stale"` rendering block:
...
- [x] Update `frontend/src/components/GalaxyMap.tsx` / `galaxyMapRender.ts` to render stale planets:
