...
- [x] Extend `WaypointTask.type` to allow `"colonize"` alongside `"transport"` and `"transfer"`
- [x] Extend `GameEvent` for colonisation events:
...
- [x] Update the file-level comment/docstring to include the new colonisation event types
...
- [x] Add `fleet_has_colony_ship(fleet: Fleet, designs_by_id: dict[str, Design]) -> bool`
- [x] Add `count_colony_ships(fleet: Fleet, designs_by_id: dict[str, Design]) -> int`
- [x] Add `recovered_minerals_for_colony_ships(count: int) -> Minerals`
- [x] Add `resolve_colonize_task(fleet: Fleet, planet: PlanetState | None, designs_by_id: dict[str, Design]) -> tuple[Fleet | None, PlanetState | None, GameEvent]`
...
- [x] Unit tests in `backend/tests/engine/test_colonisation.py`:
...
- [x] Define `COLONY_SHIP_SPEED = 6` and `COLONY_SHIP_CAPACITY = 25` alongside the existing scout and freighter constants
- [x] For each player, create a `colony_ship` design:
  - `name="Colony Ship"`
  - `hull="colony_ship"`
  - `speed=COLONY_SHIP_SPEED`
  - `cargo_capacity=COLONY_SHIP_CAPACITY`
  - `scanner=Scanner(normal=0, penetrating=0)`
- [x] For each player, create one colony ship fleet parked at the home planet with empty cargo, no waypoints, and `repeat=False`
- [x] Update comments/docstrings noting that Turn 0 now creates three default designs/fleets per player: scout, small freighter, and colony ship
- [x] Unit tests in `backend/tests/engine/test_create_game.py`:
...
- [x] Extend `_execute_waypoint_task(...)` to handle `wp.task.type == "colonize"` after the existing `transport` and `transfer` branches
...
- [x] Thread colonisation events through movement and back into `resolve_turn`
- [x] Update `move_fleet` / `move_fleets` signatures and return values as needed so dissolved fleets are removed from global state instead of being recreated as empty fleets
- [x] In `resolve_turn`, merge colonisation events into `owner_events` before mining
- [x] Unit tests in `backend/tests/engine/test_movement.py` or `backend/tests/engine/test_colonisation.py`:
...
- [x] Turn 0 owner state includes a colony ship fleet and colony ship design
- [x] Load colonists onto the colony ship with a `transport` waypoint task, then submit a `colonize` waypoint to an unowned planet; after resolving:
...
- [x] Colonising with a mixed fleet leaves non-colony ships in orbit after the colony ship is dismantled
- [x] Colonising an already owned planet emits `colonize_failed` with `reason="planet_already_owned"`
- [x] Colonising with no colonists emits `colonize_failed` with `reason="no_colonists"`
- [x] Colonising empty space emits `colonize_failed` with `reason="no_planet"`
- [x] Successful colonisation deposits dismantle recovery minerals and any remaining fleet mineral cargo onto the new colony
- [x] A newly colonised hostile planet is still claimable and is then subject to normal population death on later turns (PRD 14 behaviour)
