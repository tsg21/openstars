...
- **Fleets** — small directional icons (chevrons or triangles) at their position
...
- Their own colour
- The colour assigned to each other player (identified by username/empire name)

This matches the original Stars! approach — you picked your own colour and assigned colours to opponents for your own map clarity. Two players in the same game could see entirely different colour schemes.

Colour preferences are stored server-side in `player-preferences-{username}.yaml` (per game, alongside the other player files in GCS). This means preferences follow the player across devices — log in from another machine and your colours are there.

**Colour palette** — a preset selection of colours optimised for contrast on black, plus a custom colour picker:
...
The UI targets **desktop browsers at 1280px+ width**. This is a desktop game — no responsive layouts, no mobile breakpoints, no drawer/bottom-sheet variants.
