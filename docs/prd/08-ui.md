...
Colour preferences will be stored server-side in `player-preferences-{username}.yaml` (per game, alongside the other player files in GCS), so they follow the player across devices.
