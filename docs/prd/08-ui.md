...
Each player is assigned a colour at game creation. Colours are chosen for contrast on a black background:
...
The current player's colour is used for their planets, fleets, and scanner circles. Other players' colours appear for their visible objects.
...
The primary target is desktop browsers (1280px+ width). The layout degrades gracefully:

| Width | Behaviour |
|-------|-----------|
| ≥1280px | Full layout: map + detail panel side by side |
| 900–1279px | Detail panel overlays the map (slide-over drawer) |
| <900px | Not a priority. Map full-screen, detail panel as bottom sheet. Functional but not optimised. |
